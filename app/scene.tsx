...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  const ground=new T.Mesh(new T.CircleGeometry(30,96),new T.MeshStandardMaterial({color:0xd5d9dc,roughness:1}));ground.rotation.x=-Math.PI/2;ground.position.y=-.019;ground.visible=false;scene.add(ground);
  ...
  const createGridMarker=()=>{
    ...
    const crossPoints=[
      new T.Vector3(-crossSize,0,0),new T.Vector3(crossSize,0,0),
      new T.Vector3(0,-crossSize,0),new T.Vector3(0,crossSize,0),
      new T.Vector3(0,0,-crossSize),new T.Vector3(0,0,crossSize)
    ];
  ...
  const handleGridClick=(event)=>{
    ...
    gridMouse.x=((event.clientX-rect.left)/rect.width)*2-1;
  ...
  (window as any).__clearGridSelection=()=>{
    ...
    (window as any).__selectedGridPoint=null;
    console.log('已清除网格点选择');
  };
  ...
  return()=>{disposed=true;abort.abort();cancelAnimationFrame(frame);observer.disconnect();controls.dispose();acupointEditor.dispose();meridianLayer.dispose();geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());scene.traverse(o=>{if(o instanceof T.Mesh&&!geometries.includes(o.geometry)){o.geometry.dispose();const ms=Array.isArray(o.material)?o.material:[o.material];ms.forEach(m=>m.dispose());}});env.dispose();partTexture.dispose();selectionTexture.dispose();markerGeometry.dispose();markerMaterial.dispose();hover.remove();renderer.dispose();renderer.domElement.remove();};
 },[atlas]);
